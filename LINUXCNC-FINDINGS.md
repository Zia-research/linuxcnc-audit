...
**Who may do what.** The same file declares an access mode for every process/channel pair, and
**three process names cover the whole system** — the name is chosen in the C++ that opens the
channel, not by the program's own name.

| Declared name | `emcCommand` | `emcStatus` | `emcError` | Who opens channels under it |
|---|---|---|---|---|
| `emc` | `RW` (`:16`) | `W` (`:17`) | `W` (`:18`) | task — `emctaskmain.cc:2902`, `:2931`, `:2958` |
| `emcsvr` | `W` (`:20`) | `R` (`:21`) | `R` (`:22`) | linuxcncsvr — `emcsvr.cc:140`, `:146`, `:151` |
| `xemc` | `W` (`:26`) | `R` (`:27`) | `R` (`:28`) | AXIS — `emcmodule.cc:1642`, `:973`, `:2441` · halui — `halui.cc:292`, `:303`, `:324` · linuxcncrsh through `shcom` — `shcom.cc:76`, `:87`, `:109` |

Line numbers in the three mode columns are `configs/common/linuxcnc.nml`. `emcsvr.cc` opens the
same three channels a second time at `:172`, `:177`, `:182` — a last attempt after its ten-second
retry loop gives up, under the same name.

**Consequence 1 — the several client boxes on an architecture diagram are one client.** AXIS, halui
and linuxcncrsh do not hold separate permissions: one declaration line governs all three, and it
reads `W` command, `R` status, `R` error. Anything drawn as a client *reading* commands or *writing*
...
So a figure printing `task R` states the *usage* and a figure printing `RW` states the
*declaration*; both are defensible and they are not the same claim, which is a reason to say which
one is meant.
...
#### 2.2.1 There is a query API over the block, and the tree is being moved onto it — measured 2026-08-09

**Everything above describes HAL from the inside: a block, linked lists, offsets. That is accurate
and it is no longer the only way in.** `src/hal/hal_lib_query.c` — 1 111 lines — implements an API
through which a client asks HAL for what it holds instead of walking the structures itself. This
audit did not mention it in any of its first six weeks of work, and the file was in the tree we
read.

**What it looks like.** One struct, `hal_query_t` (`hal.h:1472-1494`), serves as both request and
reply: the caller fills in what it is looking for, HAL fills in what it found. `hal_qtype_t`
(`hal.h:1401`) names the eight kinds that can be asked for — pin, param, signal, comp, funct,
thread, thread-funct, or any. Iteration is by callback, `hal_query_cb` (`hal.h:1507`). The exported
calls, all in `hal_lib_query.c`: `hal_get_p` / `hal_set_p` (`:388`, `:182`) for pins and params,
`hal_get_s` / `hal_set_s` (`:330`, `:98`) for signals, `hal_getref_p` / `hal_getref_s` (`:531`,
`:482`) for a reference without a read, six listers — `hal_list_p` (`:613`), `hal_list_p_s`
(`:702`), `hal_list_s` (`:757`), `hal_list_comp` (`:804`), `hal_list_funct` (`:844`),
`hal_list_thread` (`:894`) — plus `hal_statistics` (`:997`) and lookup by id or name
(`hal_comp_by_id` `:1057`, `hal_comp_by_name` `:1087`).

**The locking contract is stated in the header, and it is the interesting part.** A callback runs
*with the HAL mutex held*, and the mutex is recursive so the callback may call back into the library
(`hal.h:1498`; the private recursive lock is `hal_priv.h:263`). The header's own warning is blunt
about the cost: a callback must not take long and **must not** terminate the program, or the mutex
stays locked and every other process hangs on its next HAL call. So the API does not remove the
global lock — it moves the responsibility for holding it correctly from every client into one file.

**Adoption, counted at the pinned `caa13ca6ae`.** Six files already use it: `halcmd_commands.cc`
(59 call sites), `halcmd_completion.c` (9), `meter.c` (9), `scope.c` (5), plus `scope_horiz.c` and
`scope_vert.c`. Against that, only **six files in the whole of `src/` still dereference the block
directly** through `SHMPTR`, and four of them are HAL itself — `hal_lib.c` (88), `hal_lib_query.c`
(55), `hal_priv.h` (3), `hal.hh` (1) — plus `halmodule.cc` (14), the Python binding. **The one
ordinary client left walking the structures by hand was `halrmt.c`, with 34.** `halcmd`'s residue is
ten `hal_data->` uses and they sit in one place, the instance-constructor path
(`halcmd_commands.cc:550-610`), which the query API does not cover.

**And upstream has since removed the last one.** Beyond our pin, so recorded as observation rather
than as something re-checkable in the pinned clone: `cf78a01688` (2026-08-07, Bertho Stultiens),
*"hal: Rewrite most of halrmt to become functional and use the query API"* — `halrmt.c` deleted,
`halrmt.cc` added, and the direct accesses go 34 → 0 while `hal_query` calls go 0 → 44. It belongs
to a run of commits in the same direction: `94e43de31b` introduced the API on **2026-07-23, one week
before the commit this audit is pinned to**; `07ca160b64` (07-21) made the mutex recursive, which is
what lets a callback call back in; `f9ea0910e0` (07-31) moved functs and threads to getters and
setters; and the day after halrmt, `three21kins` followed.

**Why this belongs in an audit of the architecture and not in a changelog.** The figures in this
repository draw HAL as one block that every process attaches, which remains true. What they cannot
show is that **there are now two ways to touch it** — through the structures, or through the API —
and that the project is deliberately moving its clients to the second. That is a boundary appearing
inside something this audit had described as a single shared region. *It was also visible in the
code we had already read: the omission was ours, not the tree's.*
...
> | a configuration asking for `1` with **`[TRAJ]MAX_LINEAR_JERK`** below `1.0` is silently forced back to `0` | `initraj.cc:159-162`, carrying its own `// FIXME: Should write a warning message to the user`; the runtime HAL-pin route applies the same rule at `inihal.cc:320-321` |
> | **`[TRAJ]MAX_LINEAR_JERK` defaults to `1e9`**, so the threshold above does **not** fire on a default machine — what makes a stock configuration trapezoidal is `PLANNER_TYPE`, not the jerk value | `initraj.cc:143`, `findRealV("MAX_LINEAR_JERK", "TRAJ", 1e9)` |
> | `MAX_JERK` is a **different key**, per axis and per joint, and those default to `0.0` | read at `iniaxis.cc:117` and `inijoint.cc:174`; defaults `emccfg.h:70,87` |
> | ⚠ `DEFAULT_TRAJ_MAX_JERK 0.0` at `emccfg.h:51` is **dead** — defined and read by nothing | `grep -rn DEFAULT_TRAJ_MAX_JERK src` returns the definition alone. This audit cited it as the trajectory default until 2026-08-10, which is its own rule broken on itself: a citation proves a line exists, never that anything reads it |
...
| **A class that both styles and asserts** | split it — when one class carries a colour *and* a membership claim, deleting the colour deletes the claim, and the rule that tested it goes on passing while testing nothing. The cosmetic edit and the semantic one must not share a name | 2026-08-07: cutting a figure down to a single colour scheme removed the two classes that had been doing both jobs; the containment rule then had no members left to test and reported clean. Repaired by declaring membership in classes that style nothing at all — proven by stripping them and finding no computed style changed on any element |
| **Text handed to a native program** | check the bytes that **arrived**, not the bytes you sent — a language runtime can write an encoding preamble into the pipe ahead of your first byte, and the program is blamed for it | 2026-08-13: three successive attempts to write a commit message left a UTF-8 BOM in front of the subject line. The message was clean and git was innocent; `Console.InputEncoding` was UTF-8 *with* preamble, and .NET emits it when it opens the child process's stdin |
| **A read-back through the layer that produced the write** | route the second read through a **different subsystem** — the layer that introduced a defect is the one most likely to hide it on the way back | 2026-08-13: reading that corrupted subject back through the same shell reported *first character = O*, a false green, because it silently strips a leading BOM when decoding. Only asking git itself — an anchored `--grep` carrying a witness that could fail — and then reading the raw bytes of the commit object, made the BOM visible |
...
| Date | Change |
|---|---|
| 2026-08-19 (LCNC_09) | **The 2026-08-06 correction to the context-diagram sheet was wrong in both of its last two entries, and wrong by the method this audit exists to catch.** That entry above records that the as-published view was restored to the merged file's *"(axes, spindle, IO, safety)"* and its wizard names matched to the published `PNCconf / Stepconf`. The figure the documentation actually serves says *"(joints, spindles, IO, safety)"* and `PNConf / Stepconf`. **The published state had been judged on the `.drawio` source rather than on the SVG served from it** — the two have not been the same file since [#4057](https://github.com/LinuxCNC/linuxcnc/pull/4057), where the draw.io export was replaced by a hand-rebuilt Inkscape file, and they differ in three places: the terminology (a correction, and the right one), the wizard name (`PNConf` is a letter short of the tool, `pncconf`), and the middle dots of *"Pins · Signals · Threads · Functions"*, flattened to full stops. The sheet's two drawn labels now reproduce what is served, coquille included; the 2026-08-06 entries are left standing, because rewriting them would remove the mistake rather than show it. The divergence itself is reported upstream in [#4423](https://github.com/LinuxCNC/linuxcnc/pull/4423) and left for the maintainers to settle. Two smaller repairs in the same sheet: a quotation of zz912 had been cut at *"C-morley is currently…"*, dropping the *"Part of the reason is that"* that opens his sentence, and a fourth reason of his was omitted while the text called them three. |
| 2026-08-10 (LCNC_06, last) | **The geometric figure checker was removed, and the tool that generated the figure went with it.** `diagram-check.js` measured the rendered SVG: connectors crossing boxes, overlapping text, arrows landing in open space, text hidden. It was written when the figure was hand-written SVG and *the person who could move a box was the person who could not see the result*. The figure is drawn in draw.io now, on a canvas somebody is watching, and those defects are caught there in seconds. **A check that duplicates the eye earns nothing and still has to be maintained.** With it went `drawio-to-svg.ps1`, the 280-line converter whose only remaining justification was that draw.io's own SVG export cannot be measured by that checker — the export wraps labels in `<foreignObject>` (56 of them, against 57 single-line `<text>` fallbacks for 192 rendered lines), carries no `class="grp"`, and the desktop CLI offers no option to change it: all forty-one flags were read. What remains is `drawio-check.ps1`, which reads the **model** and checks what the drawing *declares* rather than what it looks like — an edge with no `target` that appears to touch its box, a child whose declared parent its geometry contradicts, a legend asserting colour rules that do not hold. **That is the half the eye cannot do**, and it is the half that once caught this figure asserting something false about itself. |
| 2026-08-10 (LCNC_06, later) | **The reply sent to a reviewer was right before this document was. Citations 304 → 305.** Answering @grandixximo on 2026-08-09 we wrote that the 1.0 threshold guards `[TRAJ]MAX_LINEAR_JERK` and that *"`MAX_JERK` is a different key, per axis and per joint"*. That correction never reached this file, which named no key at all beside the threshold while a neighbouring row said `MAX_JERK` defaults to `0.0` — together, they read as though `MAX_JERK` were the value compared to 1.0. Two sheets said so outright. Re-read in the pinned source: `initraj.cc:143` takes `MAX_LINEAR_JERK` from `[TRAJ]` **with a default of `1e9`**, so the threshold does not fire on a default machine at all — what makes a stock configuration trapezoidal is `PLANNER_TYPE = 0`, and the conclusion (`tpCalculateSCurveAccel()` is never called) is unchanged. **And a citation of ours turned out to point at dead code**: `DEFAULT_TRAJ_MAX_JERK 0.0` at `emccfg.h:51` is defined and read by nothing, yet was pinned as the trajectory default. That is this audit's own rule broken on itself — *a citation proves a line exists, never that control flow reaches it*. The manifest gains `initraj.cc:143`, falsified like the rest. |
| 2026-08-10 (LCNC_06) | **A workshop figure was deleted, and one verification rule was taken out of it before it went.** Three local test figures, thirty of draw.io's own backups and five intermediate exports were removed from the working folder as dead weight. The longest of the three carried a lesson recorded in no other file: a class doing double duty as a colour *and* as a membership claim, so that deleting the colour silently disarmed the rule that tested membership. It is now the last row of *Verification rules earned from this audit*. The rest was checked block by block against this document first — the grouping-frame rule, the arrow-landing rule, the self-test rule and the `orient="auto"` rule were already here, and went with the files. **Nothing else in the audit moved**: no citation was added, removed or re-anchored, and the manifest stands at 304. |
| 2026-08-09 (LCNC_05) | **HAL grew an API a week before the commit this audit is pinned to, and the audit never mentioned it. New §2.2.1; citations 294 → 304.** Found by checking a reviewer's aside. grandixximo, asked whether the system-overview figure is true, answered that it looked sensible and added the caveat that master moves: *"Your halmrt box was right three days ago, Bertho has changed it since."* Checking that turned up `cf78a01688` — `halrmt.c` deleted, `halrmt.cc` added, *"Rewrite most of halrmt to become functional and use the query API"* — and one line of our figure died with it, the man-page quote *"It is not known if this interface currently works"*, which is no longer in the manual. **But the box was the small half of it.** The commit's other clause pointed at `src/hal/hal_lib_query.c`, 1 111 lines, introduced by `94e43de31b` on **2026-07-23 — one week before `caa13ca6ae`**, and therefore inside the tree this audit read. It lets a client ask HAL for pins, params, signals, comps, functs and threads instead of walking the linked lists itself. **Measured at the pin rather than assumed:** six files already used it — `halcmd_commands.cc` with 59 call sites, `halcmd_completion.c`, `meter.c`, `scope.c`, `scope_horiz.c`, `scope_vert.c` — while only six files in all of `src/` still dereferenced the block through `SHMPTR`, four of them HAL itself plus the Python binding. **The one ordinary client left walking the structures was `halrmt.c`, with 34 — the very file the reviewer named.** Upstream removed it two days later: direct accesses 34 → 0, `hal_query` calls 0 → 44. *The locking is the part worth keeping:* a callback runs **with the HAL mutex held**, and the mutex was made recursive (`07ca160b64`) so a callback may call back in — so the API does not remove the global lock, it moves the duty of holding it correctly out of every client and into one file. **What this says about the audit is the uncomfortable part.** The figures draw HAL as one block that everything attaches, which is still true; what none of them can show is that **there are now two ways to touch it**, and that the project is deliberately moving its clients to the second. A boundary is appearing inside a region this audit described as undivided — and it was legible in code already on disk. The reviewer's point generalises past his example: *master moves* is the small version; the large one is that a direction can be visible in a tree you have read and still be missed. |
| 2026-08-08 (LCNC_05) | **Six letters printed on a published figure were stated in no document of this audit. Citations 256 → 280.** The system-overview sheet draws the NML triplet as three named compartments, each carrying an access mode — *clients W · task R*, and so on. §2.1 had the three channels' size, buffer number and queue mode, and **no access column**; the word `xemc` appeared **zero times** in this file. So the claim lived on the drawing alone, where a reader can neither check it nor find what it rests on. **The fix had to be prose first.** Adding manifest entries for lines no paragraph invokes would have been the same defect mirrored — the manifest exists to back *this document*, not to accumulate anchors. §2.1 therefore gained the grid, and the twenty-four entries came after it. **The grid is 3 × 3, twice:** `configs/common/linuxcnc.nml` declares a mode for every process/channel pair under three names (`emc`, `emcsvr`, `xemc`), and for each name the C++ that opens the channels under it is now pinned as well — `emctaskmain.cc`, `emcsvr.cc`, `emcmodule.cc`, `halui.cc`, `shcom.cc`. **What the second half buys is the load-bearing part:** without it, *"AXIS, halui and linuxcncrsh are governed by one declaration line"* rests on a name a reader has to take on trust; with it, the name is a citation. **And writing the grid out produced a finding nobody was looking for:** task is the only pair declared `RW`, on `emcCommand`, and **it never uses the `W`** — across `src/emc/task/*.cc` that channel is only `read()` and `get_address()`. A figure printing `task R` and a figure printing `task RW` are making *different* claims, one about usage and one about permission, and until now this audit had never said which it meant. **All twenty-four were falsified before being trusted**: moved one line off their target, all twenty-four fail, and none of them matches its own neighbour in place. |
| 2026-08-08 (LCNC_04, fourth pass) | **The manifest goes 190 → 256 checks, and the gap that remains is a decision rather than a backlog.** The open question was whether to close the coverage gap or admit the manifest is a curated set. Measured first, because the figure everyone was repeating — *126 citations, 39 uncovered* — dated from 2026-08-07 and this document has grown since: **193 distinct `file:line` citations, 92 without an entry.** Sorting them produced a better criterion than the one this audit had been using. Not *contestable versus trivial*, but **code versus prose**. Sixty-six were added; twenty-nine were left out on purpose, and the reason for sixteen of them is the sharp part: **they point at LinuxCNC's own `.adoc` documentation, which is exactly what this audit criticises and what its own patches rewrite.** Pinning `code-notes.adoc:242` would guarantee a FAIL the day PR #4349 lands — **a check that breaks when you win is a bad check**, and it would say nothing about the code's behaviour. Ten more merely locate something a neighbouring entry already covers. What the sixty-six cover is the striking part: `motion.c:827` and `usrmotintf.cc:578`, the two halves of *created on one side, attached from the other*, **the central argument of the system-overview figure, unwatched until today**; the whole streamer/sampler section; the jerk gate at `tp.c:2759` — the threshold this project published wrongly **twice**; and the G95 chain, corrected only this morning. *The pattern is hard to miss: what made us wrong was what nothing was checking.* **Every new entry was then falsified**: pointed one line off its target, all sixty-six fail. An anchor that still matches its neighbour verifies nothing, and this is the test that says which ones do. One anchor was too loose on the first run — a shell line where a quotation mark sat between two tokens the regex expected to be adjacent — and the run caught it. The verifier's own staleness check fired as designed, refusing the run while `README.md` still promised 190. |
